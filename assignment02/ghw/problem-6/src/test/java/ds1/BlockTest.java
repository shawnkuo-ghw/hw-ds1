...
class BlockTest { 
    ...
    void testGetFirstTransaction() {
        ...
        TransactionWithFee t1 = new TransactionWithFee("A", "B", 50, 1);
        block.addTransaction(t1);
        assertEquals(t1, block.getFirstTransaction(), "First transaction should be t1");
        TransactionWithFee t2 = new TransactionWithFee("C", "D", 30, 2);
        block.addTransaction(t2);
    ...
    void testRemoveTransaction() {
        Block block = new Block(0, 2, 1);
        TransactionWithFee t1 = new TransactionWithFee("A", "B", 50,2);
        TransactionWithFee t2 = new TransactionWithFee("C", "D", 30,1);
        block.addTransaction(t1);
        block.addTransaction(t2);
        TransactionWithFee[] transactions = block.getTransactions();
        assertEquals(2, transactions.length, "There should be 2 transactions in the block");
    ...
    void testRevertTransaction() {
        ...
        block.addTransaction(t1);
        t1.revert();
        TransactionWithFee revertedT1 = block.getFirstTransaction();
        TransactionWithFee[] transactions = block.getTransactions();
