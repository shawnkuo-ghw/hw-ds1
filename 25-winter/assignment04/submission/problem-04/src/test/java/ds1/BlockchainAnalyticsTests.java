...
public class BlockchainAnalyticsTests {
    ...
    void testDetectShortestCycle() {
        ...
        // detectShortestCycle() selects the cycle with the least cost (sum of amounts)
        // so it should return the cheaper cycle: 123 -> 789 -> 456 -> 123
        String[] expectedCycleElements = {"123", "789", "456", "123"};
        for (int i = 0; i < expectedCycleElements.length; i++) {
            // check element in cycle
            assertTrue(shortestCycle2.in(expectedCycleElements[i]));
        }
        assertFalse(shortestCycle2.in("0"));
        assertFalse(shortestCycle2.in("111"));
    }
